...
export default {
  ...
  computed: {
    ...
    directors() {
      return this.movieDetail.directors ? this.movieDetail.directors.map((item: any) => item.name).join(' / ') : ' ';
    },
    casts() {
      return this.movieDetail.casts ? this.movieDetail.casts.map((item: any) => item.name).join(' / ') : ' ';
    },
    genres() {
