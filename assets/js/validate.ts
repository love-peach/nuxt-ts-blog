...
export const validatorsExp = {
  number: /^[0-9]*$/,
  nameLength: (n: number, m: number) => new RegExp(`^[\\u4E00-\\u9FA5]{${n},${m}}$`),
...
export const validatorsFun = {
  number: (val: string) => validatorsExp.number.test(val),
  idCard: (val: string) => validatorsExp.idCard.test(val),
  backCard: (val: string) => validatorsExp.backCard.test(val),
};
